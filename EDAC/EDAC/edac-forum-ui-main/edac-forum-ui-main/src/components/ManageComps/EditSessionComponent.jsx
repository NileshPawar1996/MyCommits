...
import { MANAGEMENT_URL, GET_ALL_COURSES, GET_ALL_BATCHES_BY_COURSE, GET_ALL_TEACHERS, ADD_SESSION, GET_MODULES_BY_COURSE, GET_CURRENT_SESSION, EDIT_SESSION } from '../../config'

export default function CreateSessionComponent(props) {
    ...
    function getDetails() {
        axios.get(MANAGEMENT_URL + GET_CURRENT_SESSION + props.id, {
        })
            .then(function (response) {
                debugger;
                var tempSession = response.data.data;
                setBatchesList([{ id: tempSession.batch.id, courseName: tempSession.batch.batchId }]);
                setCoursesList([{ id: tempSession.batch.course.id, batchId: tempSession.batch.course.courseName }]);
                setModuleList([{ id: tempSession.module.id, moduleName: tempSession.module.name }]);
                setSelectedTeacher([{ id: tempSession.staff.id, firstName: tempSession.staff.firstName, lastName: tempSession.staff.lastName }]);
    ...
    function submitSession() {
        ...
        // sessionObj.cou
        axios.put(MANAGEMENT_URL + EDIT_SESSION + props.id, {
            ...sessionObj
        })
            .then(function (response) {
